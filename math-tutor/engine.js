...
(function (root) {
  ...
  function expand(str) {
    ...
    // 곱셈 공식 패턴 안내 (풀이 중심)
    const compact = preprocess(str);
    let m;
    if ((m = compact.match(/^\(([^()]+)\)\^2$/))) {
      steps.push(`② 곱셈 공식 (a±b)² = a² ± 2ab + b² 를 적용합니다. (a, b는 ${m[1]}의 각 항)`);
    } else if ((m = compact.match(/^\(([^()]+)\)\(([^()]+)\)$/))) {
      steps.push(`② 두 식을 분배법칙으로 곱합니다: 앞 식의 각 항 × 뒤 식의 각 항 (FOIL).`);
      steps.push(`   합·차 공식 (a+b)(a-b)=a²-b², 또는 (x+a)(x+b)=x²+(a+b)x+ab 도 활용됩니다.`);
    } else {
      steps.push(`② 분배법칙으로 모든 항을 곱하여 전개합니다.`);
    }
    steps.push(`③ 동류항을 정리한 결과: ${polyStr(poly)}`);
  ...
  // 합이 sum, 곱이 prod 인 두 정수 찾기 (없으면 null)
  function findTwoNumbers(sum, prod) {
    const lim = Math.abs(prod) + Math.abs(sum) + 1;
    for (let m = -lim; m <= lim; m++) {
      if (m === 0 && prod !== 0) continue;
      if (prod % m !== 0 && m !== 0) continue;
      const n = sum - m;
      if (m * n === prod) return [m, n].sort((a, b) => a - b);
    }
    return null;
  }
  ...
  function factorQuadratic(str) {
    ...
    // 실제로 "곱이 ~, 합이 ~ 인 두 수"를 찾아서 보여준다 (풀이 중심)
    const two = findTwoNumbers(Math.round(B), Math.round(A * C));
    if (Math.round(A) === 1 && two) {
      steps.push(`② 곱이 c=${C}, 합이 b=${B}인 두 수를 찾습니다 →  ${two[0]} 와 ${two[1]}`);
      steps.push(`   (확인: ${two[0]}×${two[1]}=${two[0] * two[1]}, ${two[0]}+${two[1]}=${two[0] + two[1]})`);
    } else if (two) {
      steps.push(`② 곱이 ac=${A * C}, 합이 b=${B}인 두 수를 찾습니다 →  ${two[0]} 와 ${two[1]}`);
      steps.push(`   중간항 ${B}x = ${two[0]}x ${two[1] >= 0 ? '+ ' + two[1] : '- ' + (-two[1])}x 로 쪼개어 묶습니다(인수분해 공식 ax²+bx+c).`);
    } else {
      steps.push(`② 인수분해 공식을 이용해 두 일차식의 곱으로 분해합니다.`);
    }
